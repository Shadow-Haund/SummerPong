...
public class Pong implements ActionListener, KeyListener {
	...
	public void render(Graphics2D g) {
		...
		if (gameStatus == 0) {	// в меню
			...
			g.drawString("Нажми Space для игры", width / 2 - 150, height / 2 - 25);
			g.drawString("Нажми Enter для игры с ботом", width / 2 - 200, height / 2 + 25);
			g.drawString("<< Количество голов: " + scoreLimit + " >>", width / 2 - 150, height / 2 + 75);
		}

		if (gameStatus == 1) { // на паузе
			g.setColor(Color.WHITE);
			g.setFont(new Font("Arial", 1, 50));
			g.drawString("Пауза", width / 2 - 103, height / 2 - 25);
		}
		...
		if (gameStatus == 3) {	// конец игры
			...
			g.setFont(new Font("Arial", 1, 30));
			g.drawString("Нажми Space для игры", width / 2 - 150, height / 2 - 25);
			g.drawString("Нажми ESC чтобы выйти в меню", width / 2 - 220, height / 2 + 25);
		}
	}
	@Override
